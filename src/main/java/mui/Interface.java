...
public class Interface implements PConstants {
  ...
  public PGraphics createCanvas(int w, int h, String renderer, String fn) {
    PGraphics pg = app.createGraphics(w, h, renderer, fn);
    return pg;
  }  
  ...
  public void setClip(float cx, float cy, float cw, float ch) {    
    if (!currentClip.equalsTo(cx, cy, cw, ch)) {
      ...
      float[] ipx = Widget.intersect(currentClip.left(), currentClip.right(), 
                                     cx, cx + cw);
      float[] ipy = Widget.intersect(currentClip.top(), currentClip.bottom(), 
                                     cy, cy + ch);      
      if (ipx == null || ipy == null) return;
  ...
  protected class ClipRect {
    ...
    boolean equalsTo(float x, float y, float w, float h) {
      return PApplet.abs(this.x - x) < 0.01f && PApplet.abs(this.y - y) < 0.01f && 
             PApplet.abs(this.w - w) < 0.01f && PApplet.abs(this.h - h) < 0.01f;
    }
  ...
  protected void handleResize() {
    if (PApplet.abs(viewport.w - app.width) > 0 || 
        PApplet.abs(viewport.h - app.height) > 0) {
