...
//  static public int INSIDE_TOLERANCE = 1;  
  ...
  protected float xoffcoef, yoffcoef;
  protected SoftFloat xoffset, yoffset;
  ...
  protected void initWidget(Interface intf) {
    ...
    bounds = new BoundBox();
    xoffcoef = yoffcoef = 1;
    xoffset = new SoftFloat(0); 
    yoffset = new SoftFloat(0);
    xfactor = yfactor = 0;
...
//    dragParentRequested = false;
  ...
  public float left() {
    if (parent == null) {
      return bounds.x.get() + xoffcoef * xoffset.get();
    } else {
  ...
  public float top() {
    if (parent == null) {
      return bounds.y.get() + yoffcoef * yoffset.get();
    } else {
  ...
  public void setOffsetX(SoftFloat offset) {
    setOffsetX(1, offset);
  }
  
  public void setOffsetX(float coef, SoftFloat offset) {
    xoffcoef = coef;
    xoffset = offset;
  }

  public void setOffsetY(SoftFloat offset) {
    setOffsetY(1, offset);
  }
  
  public void setOffsetY(float coef, SoftFloat offset) {
    yoffcoef = coef;
    yoffset = offset;
  }  
  ...
  protected void setOrigin() {
    float x, y;
    if (parent == null) {
      x = bounds.x.get() + xoffcoef * xoffset.get();
      y = bounds.y.get() + yoffcoef * yoffset.get();
    } else {
      x = childLeft();
      y = childTop();
    }
  ...
  protected float childLeft() {
    return xfactor * parent.width() + bounds.x.get() + xoffcoef * xoffset.get();
  }
  
  protected float childTop() {
    return yfactor * parent.height() + bounds.y.get() + yoffcoef * yoffset.get();
  }
...
//  }
  ...
  public boolean visible() {
    if (isTransparent() || bounds.zero()) {
      ...
      float[] ipx = intersect(left(), right(), intf.left(), intf.right());    
      float[] ipy = intersect(top(), bottom(), intf.top(), intf.bottom());

      return ipx != null && PApplet.abs(ipx[1] - ipx[0]) > 0 && 
             ipy != null && PApplet.abs(ipy[1] - ipy[0]) > 0;      
    }
  }
...
//  }  
  ...
  public class BoundBox {
    ...
    boolean zero() {
      return PApplet.abs(w.get()) < 0.01f || PApplet.abs(h.get()) < 0.01f;      
    }
  }
  
  static public float[] intersect(float a, float b, float c, float d) {
