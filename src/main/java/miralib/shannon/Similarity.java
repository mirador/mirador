...
public class Similarity {
  ...
  static public float calculate(DataSlice2D slice, float ixy, float pvalue, Project prefs) {
    ...
    if (Float.isNaN(ixy) || Float.isInfinite(ixy)) {
      ...
      indep = gammaTest(ixy, slice.binx, slice.biny, slice.values.size(), pvalue);
    }
...
//	  double p = 0.5*(1+Erf.erf(z/Math.sqrt(2.0)));
  ...
  static protected boolean fisherTest(DataSlice2D slice, float pvalue, int binAlgo, int numTail) {
    ...
    try {
      double p = Hypergeometric.getDistribution(freqMatrix, 0, RoundingMode.HALF_UP).doubleValue();
